...
internal static class NativeLibraryLoader
{
    ...
    private static void LoadForNetFramework()
    {
        ...
        var pathsToTry = new List<string>
        {
            ...
            Path.Combine(baseDir, libFileName),
            Path.Combine(baseDir, "bin", libFileName),
            Path.Combine(baseDir, "runtimes", rid, "native", libFileName),
        };

        // On non-Windows hosts (i.e. .NET Framework running under Mono), the
        // runtime resolves a P/Invoke module by its conventional name through
        // the OS loader. Preloading via Win32 LoadLibrary is a no-op there, so
        // we only do the explicit Win32 dance on Windows and otherwise rely on
        // the native library being discoverable next to the assembly or on
