...
public sealed class TypstCompilerOptions
{
    ...
    /// <summary>
    /// Gets or sets a real on-disk root directory for the project. When set,
    /// project files (those referenced relative to the project root) that were
    /// not provided in-memory are read from this directory, mirroring the Typst
    /// CLI's <c>--root</c> option. If null, only in-memory files are served.
    /// </summary>
